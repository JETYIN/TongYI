...
//				+ obj.getText());
		...
		String contents = obj.getText();
		if (contents.contains("product-")) {
			contents = contents.substring(contents.indexOf("product-")+8 , contents.indexOf(".html"));
			startActivity(AgentActivity
					.intentForFragment(this,
							AgentActivity.FRAGMENT_GOODS_DETAIL)
							.putExtra(Run.EXTRA_SCAN_REZULT, contents));
			...
			startActivity(AgentActivity
					.intentForFragment(this,
							...
							.putExtra(Run.EXTRA_KEYWORDS, contents)
							.putExtra(Run.EXTRA_TITLE, contents));
		}
